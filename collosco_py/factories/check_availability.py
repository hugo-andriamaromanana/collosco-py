...
def test_slot(existing_kholle: Kholle, kholle: Kholle) -> bool:
    return (
        existing_kholle.slot == kholle.slot
        and existing_kholle.teacher_name == kholle.teacher_name
    )
...
def test_kholle(working_hour: ExamType, kholle: Kholle) -> bool:
    if kholle.slot.day == working_hour.slot.day:
        if not (
            kholle.slot.hours[0] > working_hour.slot.hours[0]
            and kholle.slot.hours[1] >= working_hour.slot.hours[0]
        ):
            return True
        if not (
            kholle.slot.hours[0] <= working_hour.slot.hours[0]
...
def is_group_free(trinome: Trinome, kholle: Kholle) -> bool:
    for working_hour in trinome.working_hours:
        if test_kholle(working_hour, kholle):
            return False
    return True
...
def can_place_group(
    ...
    if (
        check_if_empty(previous_weeks[0])
        and is_group_free(trinome, kholle)
        and is_teacher_too_frequent(previous_weeks, kholle)
    ):
        return True
